      ...
      .then(events => {
        ...
        events.calendar.forEach(event => {
          ...
          if (SUMMARY.importedIds[event.id]) {
            ...
            const izoneAlias = SUMMARY.importedIds[event.id].alias
            const izoneHours = SUMMARY.importedIds[event.id].hours

            if (!SUMMARY.hours[`${alias}:`]) {
              // Alias has changed.
              return SUMMARY.hours[`${izoneAlias}`].status = 'error'
            }

            if (alias !== izoneAlias) {
              // Alias has changed.
              return SUMMARY.hours[`${alias}:`].status = 'error'
            }

            if (hours !== izoneHours) {
              // Time duration differs.
              return SUMMARY.hours[`${alias}:`].status = 'warning'
            }

            // TODO: Check if start time differs.
            // TODO: Check if time entry exists only in database.

            const izoneDescription = SUMMARY.importedIds[event.id].description
            const izoneSummary = `${izoneAlias} ${izoneDescription}`
            if (event.summary !== izoneSummary) {
