...
function getCalendarEvents (izone) {
  const week = getStartAndEndOfWeek(izone.week)
  return google.getEvents(week.start, week.end, izone.google.accessToken)
}

function getIzoneEvents (izone) {
  const week = getStartAndEndOfWeek(izone.week)
  return database.getJobLogs(week.start, week.end, izone.user.p_izusername)
}
...
function roundSummaryHours () {
  Object.keys(SUMMARY.hours).map(s => {
    // Round hours to nearest .5
    SUMMARY.hours[s].hours *= 2
    SUMMARY.hours[s].hours = Math.ceil(SUMMARY.hours[s].hours)
    SUMMARY.hours[s].hours /= 2
  })
}
