...
function importGoogleCalendarEvent (event, izoneEvents, user, accessToken) {
  ...
  if (!izoneEvent) {
    ...
    if (moment(event.start.dateTime).format() !== moment(izoneEvent.jl_starttime).format()) {
      update = true
    }
...
function shouldImport (event, importTag) {
  return event.summary.indexOf(`${importTag}:`) > -1
}

module.exports = {
  import: (req, res, next) => {
    const izone = req.izone
    const user = izone.user
    const accessToken = izone.google.accessToken
    const week = izone.week || weekHelper.getWeekForDate(new Date())

    return izoneService
      .getAllEvents({ week, accessToken, username: user.p_izusername })
      .then(events => events.calendar
        .filter(event => user.p_izone_autoimport || shouldImport(event, izone.import))
        .reduce((chain, event) => {
          return chain.then(() => importGoogleCalendarEvent(event, events.izone, user, accessToken))
        }, Promise.resolve())
        .then(() => slackService.summary({ week, accessToken, username: user.p_izusername }))
        .then(data => {
          res.send(data)
          next()
        })
