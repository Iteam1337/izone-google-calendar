...
async function importGoogleCalendarEvent (event, izoneEvents, user, accessToken) {
  ...
  const izoneEvent = await databaseAdapter.getEventByGCalId(event.id)
...
module.exports = {
  import: (req, res, next) => {
    ...
    let doImportEventsMatchingAlias = null
    if (req.izone.import) {
      doImportEventsMatchingAlias = `${req.izone.import}:`
    }
    ...
    return izoneService.getAllEvents(req.izone)
      .then(events => {
        ...
        events.calendar.forEach(event => {
          let doImport = false
          if (doImportEventsMatchingAlias) {
            doImport = false
            if (event.summary.indexOf(doImportEventsMatchingAlias) > -1) {
              doImport = true
            }
          }
