...
function shouldImport (event, autoimport, importTag) {
  return autoimport || event.summary.indexOf(`${importTag}:`) > -1
}
...
module.exports = {
  import: (req, res, next) => {
    ...
    if (!izone.week) {
      izone.week = weekHelper.getWeekForDate(new Date())
    }

    return izoneService.getAllEvents(izone)
      .then(events => {
        const tasks = events.calendar
          .filter(event => shouldImport(event, user.p_izone_autoimport, izone.import))
          .map(event => ({
            event,
