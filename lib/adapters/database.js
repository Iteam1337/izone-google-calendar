...
module.exports = {
  ...
  getPersonBySlackIdentity: (slackIdentity) => {
    return query(`
      select *
      from people_db
      where p_slack_user_id = '${slackIdentity.userId}' OR p_slack_user_name = '${slackIdentity.userName}'
    `)
  },
  import: jobLog => {
    return query(`
  ...
  updatePerson: () => {
    // NOTE: Not for actual use yet!
    /*
      Insert into
      Slackuserid, token
      Where coalesce(slackuserid, aduser) = slackusername OR slackuserid
    */
  }
}
